...
server.on('upgrade', function upgrade(request, socket, head) {
  // Check if the request URL matches the verified URL
  if (request.headers.origin !== 'https://v6p9d9t4.ssl.hwcdn.net' && request.headers.origin !== 'https://serve.gamejolt.net') {
    socket.destroy();
    return;
  }
